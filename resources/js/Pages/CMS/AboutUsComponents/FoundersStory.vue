<script setup>
  ...
  const form = useForm({
    image: null,
    writeup: props.content?.founders_writeup
  })
  const filePreviewUrl = ref(props.content?.founders_photo_url ? `/storage/${props.content?.founders_photo_url}` : null)
  ...
  const handleSubmit = () => {
    ...
    form.post(route('cms.founders.updateFounders'))
  }
</script>
